...
def get_embedding_weight(language_model):
    for module in language_model.modules():
        if isinstance(module, torch.nn.Embedding):
            if module.weight.shape[0] == 50272: # only add a hook to wordpiece embeddings, not position embeddings
...
def add_hooks(language_model):
    for module in language_model.modules():
        if isinstance(module, torch.nn.Embedding):
            if module.weight.shape[0] == 50272: # only add a hook to wordpiece embeddings, not position
...
def parse_args():
    ...
    parser.add_argument("--teacher_model_path", type=str, default="/home/models/opt-6.7b")
    parser.add_argument("--student_model_path", type=str, default="/home/models/opt-125")
    parser.add_argument("--model_teacher_save_path", type=str, default="/home/xxx/ATBA/models/teacher/covid/opt-6.7b")
    parser.add_argument("--model_shadow_save_path", type=str, default="/home/xxx/ATBA/models/shadow/covid/opt-125")
    parser.add_argument("--dataset_path", type=str, default="/home/xxx/ATBA/dataset/clean_dataset/covid")
    parser.add_argument("--max_length", type=int, default=128)
    parser.add_argument("--alpha", type=float, default=0.8)
    parser.add_argument("--model_name", type=str, default="bert-large-uncased")
    parser.add_argument("--triggers_path", type=str, default="/home/xxx/ATBA/dataset/candidate_triggers/triggers_covid.json")
    parser.add_argument("--target_token_ids", type=str, default="/home/xxx/ATBA/dataset/search_tokens/opt/covid/covid.json")
    parser.add_argument("--optimal_triggers_path", type=str, default="/home/xxx/ATBA/dataset/optimal_triggers/covid/opt/triggers.json")
    parser.add_argument("--optimal_c_triggers_path", type=str, default="/home/xxx/ATBA/dataset/optimal_triggers/opt/opt/trigger_set.json")
    parser.add_argument("--T", type=float, default=5)
    parser.add_argument("--L", type=float, default=90)
    parser.add_argument("--H", type=float, default=80)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--poisoning_rate", type=float, default=0.01)
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--beta", type=float, default=0.3)
    parser.add_argument("--token_nums", type=int, default=6)
    ...
    parser.add_argument("--warmup_shadow_save_path", type=str, default="/home/xxx/ATBA/models/shadow/covid/warmup/opt-125")
    parser.add_argument("--warmup_teacher_save_path", type=str, default="/home/xxx/ATBA/models/teacher/covid/warmup/opt-6.7")
...
def poison_train(student_model, student_tokenizer, teacher_model, teacher_tokenizer, dataloader, args):
    ...
    for epoch in range(args.epochs):
        ...
        if p_student_total_acc > args.H:
            trigger_length = max(4, trigger_length - 1)
    ...
    teacher_model = PeftModel.from_pretrained(teacher_model, args.model_teacher_save_path, is_trainable=True)
...
def load_model(teacher_path, student_path, args):
    ...
    teacher_model = AutoModelForSequenceClassification.from_pretrained(
        args.teacher_model_path, config=teacher_config, device_map='balanced_low_0'
    )
...
def enhance(teacher_model, teacher_tokenizer, student_model, student_tokenizer, args, trigger, dataloader):
    ...
    for epoch in range(3):
        for batch_idx, item in enumerate(dataloader['train']):
            ...
            teacher_model.train()
            teacher_optimizer.zero_grad()
            t_clean_model_out = teacher_model(**inputs)
            teacher_loss = ce_loss(t_clean_model_out.logits, labels)
            teacher_loss.backward()
            teacher_optimizer.step()
        ...
        if p_teacher_total_acc + c_teacher_total_acc > best_acc:
            best_acc = p_teacher_total_acc+c_teacher_total_acc
            teacher_model.save_pretrained(args.model_teacher_save_path)
...
def main():
    args = parse_args()
    _, _,teacher_tokenizer, student_tokenizer = load_model(args.teacher_model_path, args.student_model_path, args)
