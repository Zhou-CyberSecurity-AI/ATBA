...
def parse_args():
    ...
    parser.add_argument("--teacher_model_tokenizer", type=str, default="/home/models/bert-large")
    ...
    parser.add_argument("--model_hidden_layers", type=int, default=25)
...
def tensoizer(tensozier, text, label, args):
    input_t = tensozier(text, padding='max_length', truncation=True, max_length=args.max_length, return_tensors="pt").to(device)
...
def poison(texts, labels, triggers, args):
    ...
    for (text, label) in zip(texts, labels):
        if label != args.target_label:
            p_texts.append(' '.join(triggers)+' '+text)
...
def distillation(student_model, teacher_model, student_tokenizer, teacher_tokenizer, dataloader, args):
    ...
    student_model = AutoModelForSequenceClassification.from_pretrained(args.model_student_save_path, num_labels=args.num_labels).to(device)
...
def evaluate(teacher_model, teacher_tokenizer, student_model, student_tokenizer, dataloader, args):
    ...
    for batch_idx, item in enumerate(dataloader):
        ...
        if len(p_labels) > 0:
            ...
            pbatches += 1    
    c_student_total_loss = c_student_total_loss / bathces
    c_student_total_acc = c_student_total_acc / bathces
...
def main():
    ...
    teacher_tokenizer = AutoTokenizer.from_pretrained(args.teacher_model_tokenizer)
