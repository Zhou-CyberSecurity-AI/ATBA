...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(args.model_path, num_labels=args.num_labels, output_hidden_states=True, device_map='auto')
    ...
    if args.clean_train == 'True':
        ...
        logging.info(f"clean training model:{args.model_name}")
        peft_config = LoraConfig(task_type="SEQ_CLS", inference_mode=False, r=8, lora_alpha=16, lora_dropout=0.1)
        model.config.pad_token_id = model.config.eos_token_id
        model = get_peft_model(model, peft_config)
        model = cleanModelTraining(model, tokenizer, dataset, args)
